...
test_expect_failure 'unexpectedly passing TODO test' '
	echo A >expected &&
...
test_expect_${TEST_LIB_OUTPUT_DEMO:-failure} 'an one-line fail (set TEST_LIB_OUTPUT_DEMO=true)' 'false'

test_expect_${TEST_LIB_OUTPUT_DEMO:-failure} 'a multi-line failure (set TEST_LIB_OUTPUT_DEMO=true)' '
...
test_expect_success 'run t0150-fake.sh' '
	...
	> not ok 4 - failing TODO test <YELLOW;BOLD># TODO known breakage<RESET>
...
test_expect_success 'run t0150-fake.sh --verbose -color' '
	...
	> <YELLOW>##<RESET>
	> Z
	> <GREEN;BOLD>not ok 4 - failing TODO test<RESET> <YELLOW;BOLD># TODO known breakage<RESET>
	> <GREEN>###<RESET>
